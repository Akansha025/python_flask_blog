...
@app.route("/about")
def about():
    return render_template('about.html', params=params)  # a: b
...
def post_route(post_slug):
    posts = Posts.query.filter_by(slug=post_slug)
    post = posts.first()
...
def edit(sno):
    if "user" in session and session['user'] == params['admin_user']:
        if request.method == "POST":
            ...
            if sno == '0':
                post = Posts(title=box_title, slug=slug, content=content, subtitle=subtitle, img_file=img_file,
                             date=date)
                db.session.add(post)
                db.session.commit()
            else:
